## we want to do go enrichment analysis locally
## load plasmoDB GO files
import pickle
import pandas as pd
import os
...
go_data=pickle.load(open('/Users/vpandey/projects/enrichmentData/pbe/gene_go_association_all.pickle','rb'))
gene_to_go=go_data[0]
go_to_id=go_data[1]
types=go_data[2]
go_to_genes=[]
for dd in gene_to_go:
    xx={}
    for k,v in dd.items():
        for val in v:
            if val not in xx.keys():
                xx[val]=[]
            xx[val].append(k)
    go_to_genes.append(xx)
...
## study
df=pd.read_csv('/Users/vpandey/projects/enrichmentData/pbe/male_only_genes.txt',sep='\t',header=None)
male_genes=df[0].to_list()
M=len(go_to_id[2].keys())
apply_enrich(go_to_genes[2],male_genes,M,'/Users/vpandey/projects/enrichmentData/pbe/test_male.txt')
