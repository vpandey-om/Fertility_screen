...
def stepwiseAnalysis():
    ...
    final_count_df,final_count_df_two_read,manfest_df=preprocessing(manifests_df,count_df)
    ...
    filtered_count_df,filtered_df_read=filter_input_dropout(final_count_df,final_count_df_two_read,input_df,manfest_df,percent)

    ######  write filtered and unfiltered files
    final_count_df_two_read.to_csv(out_folder+"/unfilterd_count_matrix_pool1.txt",sep='\t')
    filtered_df_read.to_csv(out_folder+"/filterd_count_matrix_pool1.txt",sep='\t')
    ...
    relative_growth_rate_analysis(filtered_count_df,manfest_df,prev_to_new,db_df)
