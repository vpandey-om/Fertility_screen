...
def stepwiseAnalysis(snakemake=None):
    ...
    manifests_df=pd.read_csv(snakemake.params.manifest[8],sep='\t')
    count_df=pd.read_csv(snakemake.params.barcode[8],sep='\t')
    input_df=pd.read_csv(snakemake.params.vectors[8],sep='\t')
